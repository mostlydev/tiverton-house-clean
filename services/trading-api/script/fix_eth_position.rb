...
# Fix ETH/USD position mismatch
# - Quantity: 0.09337957 (ledger) vs 0.093146125 (Alpaca)

puts "=== ETH/USD Position Fix ==="
app_root = File.expand_path("..", __dir__)

lot = PositionLot.find_by(ticker: "ETH/USD", closed_at: nil)
if lot.nil?
  puts "ERROR: No open ETH/USD position lot found"
  exit 1
end
...
ALPACA_QTY = 0.093146125
LEDGER_QTY = 0.09337957
DIFF = ALPACA_QTY - LEDGER_QTY

puts "\nAdjustment needed:"
puts "  Alpaca qty: #{ALPACA_QTY}"
puts "  Ledger qty: #{LEDGER_QTY}"
puts "  Diff: #{DIFF} ETH (~$#{(DIFF.abs * 2000).round(2)})"
